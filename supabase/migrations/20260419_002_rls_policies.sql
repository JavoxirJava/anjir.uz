...
-- =============================================================
-- YORDAMCHI FUNKSIYALAR (public schemada — auth schemaga ruxsat yo'q)
-- =============================================================
...
create or replace function public.student_class_id()
returns uuid language sql stable security definer as $$
...
create or replace function public.teacher_school_ids()
returns setof uuid language sql stable security definer as $$
  select distinct school_id from public.teacher_assignments where teacher_id = auth.uid()
$$;

create or replace function public.director_school_id()
returns uuid language sql stable security definer as $$
...
create policy "users_select_teacher_students"
  ...
  using (
    ...
    and id in (
      ...
      where class_id in (select public.teacher_class_ids())
    )
  );
  ...
  using (
    ...
    and (
      id in (
        ...
        where sp.school_id = public.director_school_id()
      )
      or id in (
        select ta.teacher_id from public.teacher_assignments ta
        where ta.school_id = public.director_school_id()
      )
    )
  );
...
create policy "schools_update_director_own"
  ...
  using (
    public.user_role() = 'director'
    and id = public.director_school_id()
  );
...
create policy "classes_insert_director_own"
  ...
  with check (
    public.user_role() = 'director'
    and school_id = public.director_school_id()
  );
...
create policy "sp_select_teacher"
  ...
  using (
    public.user_role() = 'teacher'
    and class_id in (select public.teacher_class_ids())
  );
...
create policy "sp_insert_student"
  ...
  with check (user_id = auth.uid() and public.user_role() = 'student');
...
create policy "ap_select_teacher"
  ...
  using (
    ...
    and user_id in (
      select sp.user_id from public.student_profiles sp
      where sp.class_id in (select public.teacher_class_ids())
    )
  );
...
create policy "lectures_select_student"
  ...
  using (
    ...
    and public.user_status() = 'active'
    and (
      class_id = public.student_class_id()
      or (school_id = public.student_school_id() and class_id is null)
    )
  );
...
create policy "lectures_select_teacher"
  ...
  using (
    ...
    and (
      creator_id = auth.uid()
      or class_id in (select public.teacher_class_ids())
    )
  );
...
create policy "lectures_insert_teacher"
  ...
  with check (
    ...
    and (class_id is null or class_id in (select public.teacher_class_ids()))
  );
...
create policy "tests_select_student"
  ...
  using (
    ...
    and id in (
      ...
      where class_id = public.student_class_id()
    )
  );
  ...
  using (
    ...
    and (
      ...
      or id in (
        ...
        where tc.class_id in (select public.teacher_class_ids())
      )
    )
  );
...
create policy "tests_select_director"
  on public.tests for select
  using (public.user_role() = 'director');
...
create policy "tests_insert_teacher"
  ...
  with check (public.user_role() = 'teacher' and teacher_id = auth.uid());
...
create policy "attempts_select_director"
  on public.test_attempts for select
  using (public.user_role() in ('director', 'super_admin'));
...
create policy "answers_select_teacher"
  on public.test_answers for select
  using (public.user_role() in ('teacher', 'director', 'super_admin'));
...
create policy "games_select_teacher"
  on public.games for select
  using (
    public.user_role() in ('teacher', 'director', 'super_admin')
  );
...
create policy "books_insert_teacher"
  ...
  with check (public.user_role() in ('teacher', 'director', 'super_admin'));
...
create policy "assignments_select_student"
  ...
  using (
    public.user_role() = 'student'
    and public.user_status() = 'active'
    and class_id = public.student_class_id()
  );
...
-- NOTE: leaderboard_all_time va leaderboard_weekly — VIEW (jadval emas),
-- ularga RLS qo'shib bo'lmaydi. Ular auth.uid() is not null tekshiruvi
-- ichidagi jadvallarga (test_attempts, game_attempts) tayanadi.
