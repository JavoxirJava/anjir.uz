...
router.post("/", requireRole("teacher", "super_admin"), ah(async (req: AuthRequest, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO assignments (teacher_id, topic_id, class_id, title, description, deadline, max_score, file_url, link, is_for_disabled)
...
router.post("/:id/progress", requireRole("student"), ah(async (req: AuthRequest, res) => {
  ...
  const assignmentRes = await pool.query(
    "SELECT id, class_id FROM assignments WHERE id=$1",
    [assignmentId]
  );
  const assignment = assignmentRes.rows[0] as { id: string; class_id: string } | undefined;
  if (!assignment) {
...
router.put("/submissions/:submissionId/progress-review", requireRole("teacher", "super_admin"), ah(async (req: AuthRequest, res) => {
  ...
  const subRes = await pool.query(
    `SELECT asub.id, asub.student_id, asub.progress_state, a.teacher_id
     FROM assignment_submissions asub
     JOIN assignments a ON a.id = asub.assignment_id
     WHERE asub.id=$1`,
