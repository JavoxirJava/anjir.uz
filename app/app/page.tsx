...
export default async function StudentDashboard() {
  ...
  const data = await apiGet<DashboardData>("/students/me/dashboard").catch(
    () => ({ lectures: [], tests: [], completedCount: 0, bestScore: null })
  );

  const firstName = user.first_name ?? "O'quvchi";
    ...
    <div className="space-y-8">
      <div className="rounded-2xl sm:rounded-3xl p-5 sm:p-8 gradient-primary text-white relative overflow-hidden">
        ...
        <div className="flex flex-wrap gap-4 mt-4 relative z-10">
          <div>
            <div className="text-xl sm:text-2xl font-black">{data.completedCount}</div>
            <div className="text-xs text-white/70">Yakunlangan test</div>
          </div>
          {data.bestScore !== null && (
            <div>
              <div className="text-xl sm:text-2xl font-black">{data.bestScore}%</div>
              <div className="text-xs text-white/70">Eng yaxshi ball</div>
            </div>
          )}
          <div>
            <div className="text-xl sm:text-2xl font-black">{data.lectures.length}</div>
